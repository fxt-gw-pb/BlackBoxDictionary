...
交叉验证轮流把每部分样本当作未见数据，用多次“训练—验证”近似模型对新样本的泛化表现。
...
### 2.1 根本困难
训练性能乐观，单次留出又受随机切分影响；小数据既需要训练样本也需要诚实评估。

### 2.2 关键洞察
把数据分成 $K$ 折，每次只用 $K-1$ 折拟合完整流水线，剩余一折评估；每个样本恰好获得一次 out-of-fold 预测。

### 2.3 与朴素做法对比
普通 CV 用于估计/调参；嵌套 CV 用外层评估、内层调参。重复患者、中心和时间序列必须用分组或时间切分，不能随机打散。
...
5 折验证误差为 $(0.10,0.20,0.10,0.30,0.20)$：

$$
\bar E=\frac{0.10+0.20+0.10+0.30+0.20}{5}=0.18
$$

折间样本标准差约为 0.084，说明切分造成可见波动。但折并非完全独立，不能机械地用 $0.084/\sqrt5$ 当严格置信区间。

若标准化、插补或特征选择在切分前对全数据 fit，即使模型在折内训练，验证折信息已泄漏；所有数据驱动步骤都必须放进每次训练折流水线。
...
- 按分析单位选择 KFold、Stratified、Group 或 TimeSeriesSplit。
- 全部预处理、选择和调参位于训练折。
- 重复 CV 检查切分波动。
- 超参数很多时使用嵌套 CV。
- 保存 OOF 预测以评价 AUC、校准和误差分布。
- 最终仍需锁定外部测试集或外部验证。
...
- **Fold**：一次作为验证集的子集。
- **OOF 预测**：样本未参与该次训练时得到的预测。
- **分组交叉验证**：相关样本整组进入同一折。
- **时间序列切分**：训练只使用验证时点之前的数据。
- **嵌套 CV**：外层评估、内层选择超参数。
- **流水线泄漏**：预处理提前看见验证数据。
