...
校准回答“预测 20% 风险的人是否约有 20% 发病”，关注概率的绝对可信度，而不是排序。
...
### 2.1 根本困难
模型可把高风险者排在前面却系统性高估或低估概率；AUC 对任何单调变换不变，无法发现这一问题。

### 2.2 关键洞察
把相近预测概率的人分组或平滑，在每个风险水平比较平均预测与真实发生率；理想曲线是 45° 线。

### 2.3 与朴素做法对比
Hosmer–Lemeshow 依赖任意分组且样本量敏感；校准截距、斜率、平滑曲线和 Brier score 提供更完整诊断。
...
预测概率 $(0.1,0.2,0.8,0.9)$，真实结局 $(0,1,0,1)$。分成低、高两组：
...
低风险组被严重低估，高风险组被高估。Brier score 为
$$
\frac{(0.1-0)^2+(0.2-1)^2+(0.8-0)^2+(0.9-1)^2}{4}
=0.325
$$
即使排序可能尚可，概率并不可信。
...
- 使用独立或交叉验证预测，避免训练集乐观校准。
- 同时报截距（calibration-in-the-large）与斜率。
- 用平滑曲线并附 bootstrap 置信带，避免只依赖分箱。
- 按中心、时间、亚组与风险范围检查。
- 再校准只能在独立校准集完成，并需再次外部验证。
...
- **校准截距**：整体预测是否系统偏高/偏低。
- **校准斜率**：预测是否过度极端（小于 1 常提示过拟合）。
- **Brier score**：概率预测平方误差。
- **可靠性图（Reliability diagram）**：预测与观察概率对照图。
- **再校准（Recalibration）**：在新数据上修正概率尺度。
