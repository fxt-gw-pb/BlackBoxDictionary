...
LDA 寻找“类间中心尽量远、类内样本尽量紧”的线性投影，并在共享协方差假设下形成线性分类边界。
...
原始特征中既有类别差异，也有同类患者自身波动。单纯寻找总方差最大的方向可能只看到无关个体差异，未必最利于分类。

### 2.2 关键洞察

用类间散布作为“信号”、类内散布作为“噪声”，最大化二者之比。二分类时最优方向与 $S_W^{-1}(\mu_1-\mu_0)$ 成比例：均值差越大、类内波动越小的组合获得越高判别权重。

### 2.3 投影视角与概率视角

Fisher 投影强调监督式降维；生成式 LDA 则假定各类为共享协方差的多元正态分布，通过比较类别判别函数分类。两种推导得到同一类线性边界，但解释重点不同。
...
$$
\mathcal C_0=\{(1,1),(2,1),(1,2)\},\qquad
\mathcal C_1=\{(4,4),(5,4),(4,5)\}
$$

类别均值分别是：

$$
\mu_0=\left(\frac43,\frac43\right),\qquad
\mu_1=\left(\frac{13}3,\frac{13}3\right)
$$

把两类内部的离均差外积相加，得到类内散布矩阵：
...
\propto\begin{pmatrix}1\\1\end{pmatrix}
$$

因此可取判别得分 $z=x_1+x_2$。两类投影中心分别为 $8/3$ 和 $26/3$；在等先验下，中点阈值为 $17/3\approx5.67$。新样本 $(3,2)$ 的得分为 5，低于阈值，故判为 $\mathcal C_0$。这个计算展示了 LDA 不是分别给每个变量定阈值，而是先构造最有判别力的线性组合。
...
- **查看类内分布**：分类别画直方图、Q-Q 图和散点矩阵；严重偏态、厚尾和异常值会影响均值与协方差。
- **比较协方差结构**：检查各类协方差/相关矩阵；差异明显时比较 QDA、正则化判别或 Logistic 回归。
- **检查矩阵稳定性**：特征数接近样本数、变量高度共线时，报告条件数并使用 shrinkage LDA、降维或变量筛选。
- **设定目标先验**：训练样本类别比例不等于临床患病率时，应按目标人群设先验，观察边界与后验概率变化。
- **在折内完成预处理**：标准化、筛选、PCA 和协方差估计均只能使用训练折。
- **量化方向稳定性**：用 bootstrap 或重复交叉验证观察判别系数符号、排序及得分分离是否稳定。
- **完整评价预测**：在独立数据报告混淆矩阵、AUC、校准和临床净获益，不能只展示训练集 LD 图。
...
- **类内散布**：同一类别样本围绕本类中心的变异。
- **类间散布**：不同类别中心相对总体中心的分离程度。
- **Fisher 判别准则**：类间散布与类内散布之比。
- **判别方向**：使 Fisher 准则最大的线性投影向量。
- **共享协方差**：所有类别使用同一协方差矩阵的生成式假设。
- **判别得分**：样本在线性判别轴上的投影值。
- **Shrinkage LDA**：把不稳定样本协方差向结构化目标收缩的正则化版本。
- **类别先验**：分类前预期各类别出现的概率。
