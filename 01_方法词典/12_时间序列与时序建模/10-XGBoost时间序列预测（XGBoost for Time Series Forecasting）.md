...
XGBoost 时序预测先把时间序列改造成滞后、滚动和日历特征表，再用梯度提升树学习非线性规则。
...
树模型本身不知道顺序。关键洞察是把“截至预测时点可见的历史”显式编码成监督特征；模型负责非线性和交互，研究者负责阻止未来信息进入特征。
...
某时点前三期为 $(10,12,15)$，则 lag1=15、lag2=12、三期均值 $=12.33$。若一棵树规则为“lag1 大于 14 加 3，否则减 1”，基准预测 12，则
$$
\hat y=12+3=15
$$
滚动均值必须只用当前时点之前的数据；若包含待预测值就发生泄漏。
...
使用 walk-forward 验证；审计所有 rolling/target encoding 的时间边界，比较 naive/ARIMA 基线，检查漂移、递归多步误差、特征重要性稳定性和预测区间。
...
- **滞后特征（Lag feature）**：预测时点之前的观测值。
- **滚动统计（Rolling statistic）**：仅用历史窗口计算的均值、极值等。
- **Walk-forward validation**：保持时间顺序逐步向前回测。
- **目标泄漏（Target leakage）**：特征含有预测时不可获得的信息。
