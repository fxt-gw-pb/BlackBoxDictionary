...
LSTM 预测把过去窗口逐步压入门控记忆，再从最后状态输出未来值；真正关键的是无泄漏窗口构造与时间回测。
...
线性 ARIMA 难表示阈值、交互和长依赖。LSTM 用遗忘、输入与输出门动态保存历史，但并不自动理解时间间隔、季节或因果关系，且需要比经典模型更多数据。
...
设简化细胞更新 $c_t=0.8c_{t-1}+0.5\tilde c_t$，$c_0=0$，三期候选为 $(2,-1,3)$：
$$
c_1=1,\quad c_2=0.8-0.5=0.3,\quad c_3=0.24+1.5=1.74
$$
若输出层为 $\hat y_{t+1}=10+2c_t$，则下一期预测 $13.48$。门控使旧信号衰减并与新信号合并。
...
严格按时间切分，缩放器只拟合训练段；比较不同窗口、随机种子和简单基线，检查多步递归误差、缺失 mask、预测区间与漂移。
...
- **滑动窗口（Sliding window）**：把过去 $L$ 步转为一个监督样本。
- **直接多步预测**：一次输出多个未来步。
- **递归预测**：把前一步预测继续作为输入。
- **教师强制（Teacher forcing）**：训练时使用真实前一步而非模型预测。
