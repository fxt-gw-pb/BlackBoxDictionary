...
Holt-Winters 用指数衰减权重在线更新水平、趋势和季节三个状态，并把它们外推到未来。
...
固定窗口平均会突然丢弃旧数据，也无法分离趋势与季节。指数平滑让近期观测权重更高，并把序列压缩成少量可递推状态；新数据到来时无需重新拟合全部历史。
...
无季节的 Holt 更新中，旧水平 $l_{t-1}=100$、旧趋势 $b_{t-1}=4$、新观测 $y_t=110$，取 $\alpha=0.5,\beta=0.25$：
$$
l_t=0.5(110)+0.5(104)=107
$$
$$
b_t=0.25(107-100)+0.75(4)=4.75
$$
下一期预测为 $107+4.75=111.75$。季节版再加上或乘上对应季节状态。
...
比较加法/乘法季节、阻尼趋势和不同周期；检查残差 ACF、异常值、区间覆盖及滚动验证。季节幅度随水平增大时优先乘法形式。
...
- **水平（Level）**：当前去除趋势季节后的基线。
- **趋势（Trend）**：水平每期变化速度。
- **季节状态（Seasonal state）**：周期位置的重复偏离。
- **阻尼趋势（Damped trend）**：让远期趋势逐渐趋平。
